	...
	if (_renderType != RenderType::None)
	{
		if (_renderType == RenderType::D3D12)
		{
			...
			if (_renderType == RenderType::D3D12)
			{
...
#if _M_X64
				g_methodsTable = (uint64_t*)::calloc(161, sizeof(uint64_t));
				...
				memcpy(g_methodsTable + 44 + 19 + 9 + 60 + 18, *(uint64_t**)fence, 11 * sizeof(uint64_t));
#elif defined _M_IX86
				g_methodsTable = (uint32_t*)::calloc(161, sizeof(uint32_t));
				...
				memcpy(g_methodsTable + 44 + 19 + 9 + 60 + 18, *(uint32_t**)fence, 11 * sizeof(uint32_t));
#endif
