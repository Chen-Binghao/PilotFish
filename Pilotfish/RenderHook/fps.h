...
void Setkernel_num()
{
	kernelnumData = to_string(int(kernelnum));
	strcpy((char*)kernelnumBuffer, kernelnumData.c_str());
	return;
}
	...
	while (true)
	{
		...
		else
		{
			...
			if (a->extra1)
			{
				...
				if (gamelog)
				{
					if (fps > 60) kernelnum *= 1.2;
					if (fps < 60) kernelnum *= 0.5;
					fprintf(rendertime_f, "%f,%d,%f,%f,%d, ,%f,%d,%f,%d,%d\n", realtime, framenum, render, fps, GetGpuRate_byte(),
						kernel_launchtime, kernel_launchnum, kernel_remaintime, kernel_remainnum, kernelnum);
					setbusy(false);
					settime((16.6 - render));
					Setkernel_num();
					setflip();
...
int dx12Thread()
{
	...
	kernelnumMap = ::CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0,
		MAP_SIZE,
		L"kernelnum");
	...
	kernelnumBuffer = ::MapViewOfFile(kernelnumMap, FILE_MAP_ALL_ACCESS, 0, 0, VIEW_SIZE);
	...
	strcpy((char*)kernelnumBuffer, kernellaunchtimeData.c_str());
	setbusy(true);
